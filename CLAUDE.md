...
- The GitHub reviewer reads `GITHUB_TOKEN` directly from `process.env` and falls back to `core.getInput("github-token")`. Similarly, `ANTHROPIC_API_KEY` is read directly from `process.env` (with a `core.getInput` fallback). The generated workflow therefore sets the standard env var names (`GITHUB_TOKEN`, `ANTHROPIC_API_KEY`) rather than the `INPUT_*` format that `core.getInput` expects.
